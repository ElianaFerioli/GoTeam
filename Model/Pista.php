...
class Pista {
  ...
  public static function getNomInstPorDeporteParaEvento($valorDeporte, $codEven) {
    $conexion = GoTeamDB::connectDB();
    $seleccion = "SELECT DISTINCT(NomInst), A.CodInst FROM PISTA A, INSTALACION B, EVENTO E WHERE A.CodInst=B.CodInst AND "
            . "A.CodDep=".$valorDeporte." AND A.CodPista=E.CodPista AND E.CodEven=".$codEven;
    $consulta = $conexion->query($seleccion);
    $instalaciones= [];    
    while ($registro = $consulta->fetchObject()) {
        $instalaciones[] = $registro->NomInst;
  ...
  public static function getNomInstPorDeporteParaEventoIndex($valorDeporte) {
    $conexion = GoTeamDB::connectDB();
    $seleccion = "SELECT DISTINCT(NomInst), A.CodInst FROM PISTA A, INSTALACION B, EVENTO E WHERE A.CodInst=B.CodInst AND "
            . "A.CodDep=".$valorDeporte." AND A.CodPista=E.CodPista";
    $consulta = $conexion->query($seleccion);
    $instalaciones= [];    
    while ($registro = $consulta->fetchObject()) {
        $instalaciones[] = $registro->NomInst;
