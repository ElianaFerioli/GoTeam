...
class Usuario {
  ...
  public static function updateAtributo($attr, $valor, $email) {
    $conexion = GoTeamDB::connectDB();
    $update = "UPDATE USUARIO SET ".$attr."= '".$valor."' ". 
            "WHERE EmailUs='".$email."'";
    $conexion->exec($update);
  }
  ...
  public static function verificaUsuario($email){
    $existe = false;
    $conexion = GoTeamDB::connectDB();
    $consulta = "SELECT count(*) from USUARIO where EmailUs=\"".$email."\"";
  ...
  public static function verificaPassword($email){
    $conexion = GoTeamDB::connectDB();
    $seleccion = "SELECT PassUs from USUARIO where EmailUs=\"".$email."\"";
  ...
  public static function verificaAdmin($nomUs){
    $conexion = GoTeamDB::connectDB();
    $seleccion = "SELECT Admin from USUARIO where EmailUs=\"".$nomUs."\"";
    $consulta = $conexion->query($seleccion);    
    $resultado = false;    
    while ($registro = $consulta->fetchObject()) {
        if($registro->Admin==1){
